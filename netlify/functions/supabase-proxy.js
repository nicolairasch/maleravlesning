exports.handler = async (event) => {
  ...
  // For POST on maler_avlesninger, add on_conflict for upsert
  let urlPath = path;
  if (method === 'POST' && path.startsWith('maler_avlesninger') && !path.includes('on_conflict')) {
    urlPath = path.includes('?')
      ? `${path}&on_conflict=meter_id,year,month`
      : `${path}?on_conflict=meter_id,year,month`;
  }
  ...
  return {
    statusCode: res.ok ? res.status : 200,
